#ifndef YM_TUBEADJ_DISC_LONG_C
#define YM_TUBEADJ_DISC_LONG_C
...
void real_main(char *in_file)
    ...
    for(count=2; count<STDIM; count++)
       ...
       if(tmp!= param.d_size[count])
         {
         fprintf(stderr, "When using yang_mills_tubeadj_disc_long all the spatial sizes have to be of equal length.\n");
         exit(EXIT_FAILURE);
         }
       }
    ...
    if(param.d_start != 2) // NEW SIMULATION
      ...
      write_tubeadj_disc_stuff_on_file(&GC, &param, 0);
      }
      ...
      read_tubeadj_disc_stuff_from_file(&GC, &param, &iteration);
      ...
      if(iteration<0) // update the conf, no multilevel
        ...
        {
        multilevel_tubeadj_disc_long(&GC,
                                     &geo,
                                     &param,
                                     param.d_ml_step[0],
                                     iteration);
        ...
        if(iteration==param.d_ml_level0_repeat)
          {
          // print the measure
          perform_measures_tubeadj_disc_long(&GC, &param, datafilep);

          iteration=-1; // next time the conf will be updated, no multilevel
          }

        // save multilevel stuff
        write_tubeadj_disc_stuff_on_file(&GC, &param, iteration);
        }
      }
    time(&time2);
    // montecarlo end
