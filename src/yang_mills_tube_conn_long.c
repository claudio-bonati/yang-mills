...
void real_main(char *in_file)
    ...
    alloc_tube_conn_stuff(&GC, &geo, &param);
    ...
    if(param.d_start != 2) // NEW SIMULATION
      ...
      write_tube_conn_stuff_on_file(&GC, &geo, &param, 0);
      }
      ...
      read_tube_conn_stuff_from_file(&GC, &geo, &param, &iteration);
      ...
      if(iteration<0) // update the conf, no multilevel
        ...
        if(iteration==param.d_ml_level0_repeat)
          {
          // print the measure
          perform_measures_tube_conn_long(&GC, &geo, &param, datafilep);

          iteration=-1; // next time the conf will be updated, no multilevel
          }

        // save multilevel stuff
        write_tube_conn_stuff_on_file(&GC, &geo, &param, iteration);
        }
      }
    time(&time2);
    // montecarlo end
    ...
    free_tube_conn_stuff(&GC, &geo, &param);
