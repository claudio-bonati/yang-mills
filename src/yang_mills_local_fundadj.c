#ifndef YM_LOCAL_FUNDADJ_C
#define YM_LOCAL_FUNDADJ_C
...
void real_main(char *in_file)
    ...
    for(count=1; count < param.d_sample + 1; count++)
       {
       update_fundadj(&GC, &geo, &param, &acc_local);
       acc+=acc_local;

       if(count % param.d_measevery ==0 && count >= param.d_thermal)
         {
         perform_measures_localobs_fundadj(&GC, &geo, &param, datafilep);
         }
    ...
    print_parameters_local_fundadj(&param, time1, time2, acc);
