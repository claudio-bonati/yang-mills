...
// print on screen the values of the topological charge
// computed after cooling
void topcharge_cooling_print_on_screen(Gauge_Conf const * const GC,
                           GParam const * const param,
                           int cooling_max,
                           int cooling_step)
   {
   ...
   for(i=0; i<cooling_max/cooling_step; i++)
      {
      cooling(&helperconf, param, cooling_step);

      ris=topcharge(&helperconf, param);

      plaquette(&helperconf, param, &plaqs, &plaqt);

      printf("%d  %lf  %lf\n", (i+1)*cooling_step, ris, 0.5*(plaqs+plaqt) );
      fflush(stdout);
      }
...
// print on screen the values of the topological charge
// computed after gradient flow
void topcharge_gradflow_print_on_screen(const Gauge_Conf * const GC,
                            const GParam * const param,
                            double t_max,
                            double t_step)
   {
   Gauge_Conf helperconf;
   double ris, plaqs, plaqt;
   const double dt=0.02; // integration step
   double tot_t, taux;

   init_gauge_conf_from_gauge_conf(&helperconf, GC, param);
   // helperconf is a copy of the configuration

   printf("\nGradient flow measurements\n");
   tot_t=0.0;
   while(tot_t<t_max)
     {
     taux=0.0;
     while(taux<t_step)
      {
      gradflow_RKstep(&helperconf, param, dt);
      tot_t+=dt;
      taux+=dt;
      }
...
// print on screen the values of the topological charge
// computed after ape smearing
void topcharge_ape_smear_print_on_screen(Gauge_Conf const * const GC,
                             GParam const * const param,
                             int n_max,
                             int n_step,
                             double alpha)
   ...
   printf("\nApe smearing measurements with parameter %.4lf\n", alpha);
   for(i=0; i<n_max/n_step; i++)
      {
      ape_smearing(&helperconf, param, alpha, n_step);

      ris=topcharge(&helperconf, param);

      plaquette(&helperconf, param, &plaqs, &plaqt);

      printf("%d  %lf  %lf\n", (i+1)*n_step, ris, 0.5*(plaqs+plaqt) );
      fflush(stdout);
      }
...
// print on screen the values of the topological charge
// computed after ape smearing  WITHOUT UNITARIZATION
void topcharge_ape_smear_nounit_print_on_screen(Gauge_Conf const * const GC,
                                    GParam const * const param,
                                    int n_max,
                                    int n_step,
                                    double alpha)
   ...
   printf("\nApe smearing without unitarization measurements with parameter %.4lf\n", alpha);
   for(i=0; i<n_max/n_step; i++)
      {
      ape_smearing_nounit(&helperconf, param, alpha, n_step);
