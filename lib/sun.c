...
// questa funzione massimizza l'operatore X(n) e calcola la matrice trasformazione di gauge G(n) -- CASO SU(N)
// ref C. Bonati, M. D'Elia Nuc. Phys. B 877 (2013) 233-259 
void max_X_comp_G_SuN (double OverRelaxParam,
                       SuN *X,
                       SuN *G)
   { 
   Su2 X_subgroup, G_su2;
   int i, j, k;
   double complex gii, gij, gji, gjj;
   ...
   for (i=0; i<NCOLOR-1; i++)
       ...
       for (j=0; j<NCOLOR; j++)
           {
           //massimizzo il funzionale per il sottogruppo Su2 riga i colonna j
           ennetodue_bis(X, i, j, &X_subgroup);
           max_X_comp_G_Su2_aux(OverRelaxParam, &X_subgroup, &G_su2);

           //applico la matrice G_su2 all'operatore X
           gii  =  G_su2.comp[0] + G_su2.comp[3]*I;
           gij  =  G_su2.comp[2] + G_su2.comp[1]*I;
           gji  = -G_su2.comp[2] + G_su2.comp[1]*I;
           gjj  =  G_su2.comp[0] - G_su2.comp[3]*I;
           
           // X -> G*X*G^{\dag}
           for (k=0; k<NCOLOR; k++)
               {
               temp0 = X->comp[m(k,i)]*conj(gii) + X->comp[m(k,j)]*conj(gij);
               temp1 = X->comp[m(k,i)]*conj(gji) + X->comp[m(k,j)]*conj(gjj); 
               
               X->comp[m(k,i)] = temp0;
               X->comp[m(k,j)] = temp1;
 
               temp0 = X->comp[m(i,k)]*gii + X->comp[m(j,k)]*gij;
               temp1 = X->comp[m(i,k)]*gji + X->comp[m(j,k)]*gjj; 
                
               X->comp[m(i,k)] = temp0;               
               X->comp[m(j,k)] = temp1;
               }

           //costrusico la matrice G come il prodotto delle G_su2 riportate in SU(N)
           for (k=0; k<NCOLOR; k++)
               {
               temp0 = G->comp[m(k,i)]*gii + G->comp[m(k,j)]*gji;
               temp1 = G->comp[m(k,i)]*gij + G->comp[m(k,j)]*gjj; 
