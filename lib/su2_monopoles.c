...
void comp_MAG_gauge_transformation_Su2(Su2 X_links[2*STDIM],
   ...
   // Convert the matrix X in the Pauli matrices form
   vec_x[0] = creal(X[1]);
   vec_x[1] = cimag(X[2]);
   vec_x[2] = creal(X[0]);
...
// and returns also the gauge transformation that implement the diagonalization
...
// compute the norm of the out-of-diagonal elements of X
...
void comp_functional_fmag_Su2(Su2 X_links[2*STDIM], 
   ...
   int dir, i;
   Su2 U, Udag;
   complex double F[4], U_matrix[4], Udag_matrix[4], aux1[4], aux2[4];
   ...
   *fmag = creal(F[0]) + cimag(F[0]) + creal(F[3]) + cimag(F[3]);
   }
