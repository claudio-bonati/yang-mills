...
// multilevel for polyakov correlator in the adjoint representation to be used in long simulations
void multilevel_polycorradj_long(Gauge_Conf * GC,
  ...
  if(dt!=param->d_ml_step[0])
    {
    fprintf(stderr, "'dt' has to be equal to ml_step[0] in multilevel_polycorradj_long (%s, %d)\n", __FILE__, __LINE__);
    exit(EXIT_FAILURE);
    }
  ...
  for(upd=0; upd< param->d_ml_upd[0]; upd++)
     ...
     update_for_multilevel(GC, geo, param, 0);
       ...
       for(raux=0; raux<param->d_space_vol*param->d_size[0]/param->d_ml_step[1]; raux++)
          ...
          zero_TensProdAdj(&(GC->ml_polycorradj[1][slice][r]));
          }
       ...
       // update polycorradj[0] with polycorradj[1]
       #ifdef OPENMP_MODE
       #pragma omp parallel for num_threads(NTHREADS) private(raux)
       #endif
       for(raux=0; raux<param->d_space_vol*param->d_size[0]/param->d_ml_step[0]; raux++)
  ...
  if(iteration==param->d_ml_level0_repeat-1) // iteration starts from zero
    ...
    for(raux=0; raux<param->d_space_vol*param->d_size[0]/param->d_ml_step[0]; raux++)
       ...
       times_equal_real_TensProdAdj(&(GC->ml_polycorradj[0][slice][r]), 1.0/((double) param->d_ml_upd[0] * (double) param->d_ml_level0_repeat) );
       }
    }
  } // end of multilevel
...
// compute polyakov loop in the adjoint representation and plaquette on a single slice
...
// multilevel for flux width computation in the adjoint
// representation using the disconnected correlator
void multilevel_tubeadj_disc(Gauge_Conf * GC,
  ...
  switch(level)
    ...
    case -1 :     // LEVEL -1, do not average
      ...
      // initialyze ml_polycorradj[0] and ml_polyplaqadj[0] to 0
      #ifdef OPENMP_MODE
      #pragma omp parallel for num_threads(NTHREADS) private(raux)
      #endif
      for(raux=0; raux<param->d_space_vol*param->d_size[0]/param->d_ml_step[0]; raux++)
         ...
         if(slice==0)
           {
           zero_TensProdAdj(&(GC->ml_polyplaqadj[0][r]));
           }
         }
    ...
    case NLEVELS-1 : // INNERMOST LEVEL
      ...
      for(upd=0; upd< param->d_ml_upd[level]; upd++)
         ...
         // compute the tensor products
         // and update ml_polycorradj[level] and ml_polyplaqadj[level]
         #ifdef OPENMP_MODE
         #pragma omp parallel for num_threads(NTHREADS) private(raux)
         #endif
         for(raux=0; raux<param->d_space_vol*param->d_size[0]/param->d_ml_step[level]; raux++)
            ...
            if(slice==0)
              {
              times_equal_real_TensProdAdj(&TP, creal(GC->loc_plaq[r])); // only the real part of the plaquette can contribute
                                                                         // since the trace is real in the adjoint representation
              plus_equal_TensProdAdj(&(GC->ml_polyplaqadj[level][r]), &TP);
              }
            }

        } // end of update

      // normalize polycorradj and polyplaqadj
      #ifdef OPENMP_MODE
      #pragma omp parallel for num_threads(NTHREADS) private(raux)
      #endif
      for(raux=0; raux<param->d_space_vol*param->d_size[0]/param->d_ml_step[level]; raux++)
         ...
         if(slice==0)
           {
           times_equal_real_TensProdAdj(&(GC->ml_polyplaqadj[level][r]), 1.0/(double) param->d_ml_upd[level]);
           }
         }
    ...
    default:  // NOT THE INNERMOST NOT THE OUTERMOST LEVEL
      ...
      for(upd=0; upd< param->d_ml_upd[level]; upd++)
         ...
         // initialyze ml_polycorradj[level+1] and ml_polyplaqadj[level+1] to 0
         #ifdef OPENMP_MODE
         #pragma omp parallel for num_threads(NTHREADS) private(raux)
         #endif
         for(raux=0; raux<param->d_space_vol*param->d_size[0]/param->d_ml_step[level+1]; raux++)
            ...
            if(slice==0)
              {
              zero_TensProdAdj(&(GC->ml_polyplaqadj[level+1][r]));
              }
            }
         ...
         // update polycorradj[level] with polycorradj[level+1]
         // and analogously for polyplaqadj
         #ifdef OPENMP_MODE
         #pragma omp parallel for num_threads(NTHREADS) private(raux)
         #endif
         for(raux=0; raux<param->d_space_vol*param->d_size[0]/param->d_ml_step[level]; raux++)
      ...
      // normalize polycorradj[level] and polyplaqadj[level]
      #ifdef OPENMP_MODE
      #pragma omp parallel for num_threads(NTHREADS) private(raux)
      #endif
      for(raux=0; raux<param->d_space_vol*param->d_size[0]/param->d_ml_step[level]; raux++)
...
// multilevel for flux width computation in the adjoint
// representation using the disconnected correlator
// to be used in long simulations
void multilevel_tubeadj_disc_long(Gauge_Conf * GC,
  ...
  if(dt!=param->d_ml_step[0])
    {
    fprintf(stderr, "'dt' has to be equal to ml_step[0] in multilevel_tubeadj_disc_long (%s, %d)\n", __FILE__, __LINE__);
    exit(EXIT_FAILURE);
    }

  // initialyze ml_polycorradj[0] and ml_polyplaqadj[0] to 0 if needed
  if(iteration==0)
    {
    #ifdef OPENMP_MODE
    #pragma omp parallel for num_threads(NTHREADS) private(raux)
  ...
  for(upd=0; upd < param->d_ml_upd[0]; upd++)
     ...
     update_for_multilevel(GC, geo, param, 0);
       ...
       // compute the tensor products
       // and update ml_polycorradj[0] and ml_polyplaqadj[0]
       #ifdef OPENMP_MODE
       #pragma omp parallel for num_threads(NTHREADS) private(raux)
       #endif
       for(raux=0; raux<param->d_space_vol*param->d_size[0]/param->d_ml_step[0]; raux++)
       ...
       for(raux=0; raux<param->d_space_vol*param->d_size[0]/param->d_ml_step[1]; raux++)
          ...
          if(slice==0)
            {
            zero_TensProdAdj(&(GC->ml_polyplaqadj[1][r]));
            }
          }
  ...
  if(iteration==param->d_ml_level0_repeat-1) // iteration starts from zero
    ...
    for(raux=0; raux<param->d_space_vol*param->d_size[0]/param->d_ml_step[0]; raux++)
       ...
       if(slice==0)
         {
         times_equal_real_TensProdAdj(&(GC->ml_polyplaqadj[0][r]), 1.0/((double) param->d_ml_upd[0] * (double) param->d_ml_level0_repeat) );
         }
       }
    }
  } // end of multilevel
