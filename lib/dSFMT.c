...
#include <string.h>
#include <stdlib.h>
#include "../include/dSFMT-params.h"
#include "../include/dSFMT-common.h"

/** dsfmt internal state vector */
dsfmt_t dsfmt_global_data;
/** dsfmt mexp for check */
static const long dsfmt_mexp = DSFMT_MEXP;
...
inline static void gen_rand_array_o0o1(dsfmt_t *dsfmt, w128_t *array,
				       long size);
inline static long idxof(long i);
...
void dsfmt_chk_init_gen_rand(dsfmt_t *dsfmt, uint32_t seed, long mexp) {
    ...
    for (i = 1; i < (DSFMT_N + 1) * 4; i++) {
        psfmt[idxof(i)] = (uint32_t) 1812433253UL
        * (psfmt[idxof(i - 1)] ^ (psfmt[idxof(i - 1)] >> 30)) + (uint32_t)i;
    }
    initial_mask(dsfmt);
...
void dsfmt_chk_init_by_array(dsfmt_t *dsfmt, uint32_t init_key[],
    ...
    long lag;
    long mid;
    long size = (DSFMT_N + 1) * 4;	/* pulmonary */
