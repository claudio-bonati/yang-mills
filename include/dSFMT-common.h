...
inline static void do_recursion(w128_t *r, w128_t *a, w128_t * b,
    ...
    const vector unsigned long sl1_msk = ALTI_SL1_MSK;
    const vector unsigned char sr1 = ALTI_SR;
    const vector unsigned char sr1_perm = ALTI_SR_PERM;
    const vector unsigned long sr1_msk = ALTI_SR_MSK;
    const vector unsigned char perm = ALTI_PERM;
    const vector unsigned long msk1 = ALTI_MSK;
    vector unsigned long w, x, y, z;

    z = a->s;
    w = lung->s;
    x = vec_perm(w, (vector unsigned long)perm, perm);
    y = vec_perm(z, (vector unsigned long)sl1_perm, sl1_perm);
    y = vec_sll(y, sl1);
    y = vec_and(y, sl1_msk);
    w = vec_xor(x, b->s);
    w = vec_xor(w, y);
    x = vec_perm(w, (vector unsigned long)sr1_perm, sr1_perm);
    x = vec_srl(x, sr1);
    x = vec_and(x, sr1_msk);
    y = vec_and(w, msk1);
    z = vec_xor(z, y);
